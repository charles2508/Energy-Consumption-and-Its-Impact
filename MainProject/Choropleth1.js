...
function regionalEnergyConsumption(groupByYear2019, groupByCountry) {
    var projection = d3.geoMercator() //Specify type of projection
                    .center([55, 0 ]) //Make the map appears at the center of the svg canvas
                         //.translate([w/2, h/2]) //Translate the map to left with w/2 units and to the top with h/2 units
                        .scale(120); //Scale the size of the map 
    ...
    var choropleth_w = 800;
    var choro_pleth_h = 800;
          ...
          .on("mouseover", function(event,d) {
               //----------FIRST HIGHLIGHT THE COUNTRY--------------------------------
                ...
                d3.select(this)
                    .style("opacity", 1)
                    .append('title')
                    .text(d.properties.name + " consumped " +d.properties.value +    " TWh in 2019");
               mouseOverMap(d);
           })//mouse over trigger
