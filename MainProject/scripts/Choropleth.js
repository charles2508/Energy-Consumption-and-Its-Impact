...
function regionalEnergyConsumption(groupByYear2019, groupByCountry) {
    ...
    //zoom function
    svg.call(d3.zoom().scaleExtent([1, 5]).translateExtent([[10, -100],[900, 630]]).on('zoom', (event) => {
        console.log('zoom');
...
d3.json("json/countries.json").then(function(json) {
    ...
    const legendLinear = d3.legendColor()
                        .shapeWidth(30)
                        .cells([0,1600, 3200, 4800, 6400, 8000, 9600, 11200, 12800, 14400])
                        .orient('vertical')
                        .scale(color);
    ...
    var highest_cell = svg.append('rect')
                        .attr('x', 30)
                        .attr('y', 0)
                        .attr('width', 30)
                        .attr('height', 15)
    ...
    var nodata_cell = svg.append('rect')
                    .attr('x', 30)
                    .attr('y', 0)
                    .attr('width', 30)
                    .attr('height', 15)
                    .style('fill', 'grey')
                    .attr("class", "cell")
                    .attr("transform", "translate(0, 509)");
    svg.append('text')
                .attr('y', 521)
                .attr('x', 70)
                .text('Missing data')
